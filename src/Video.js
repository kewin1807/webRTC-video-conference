...
class Video extends Component {
	constructor(props) {
		...
		this.roomnames = window.location.href.split("/")
		this.roomname = this.roomnames[this.roomnames.length - 1]
		...
		this.state = {
			...
			password: "",
			newmessages: 0,
			username: props && props.location.state && props.location.state.username ? props.location.state.username : faker.internet.userName(),
			askForUsername: props && props.location.state && props.location.state.username ? false : true,
	...
	connectToSocketServer = () => {
		...
		socket.on('connect', () => {
			const password = this.props.location.state && this.props.location.state.password ? this.props.location.state.password : this.state.password
			socket.emit('join-call', window.location.href, password)
			socketId = socket.id
	...
	connect = () => {
		...
		socket.on("connect", () => {
			socket.emit('auth', window.location.href, this.state.password);
				...
				{this.state.askForUsername === true ?
					<div>
						<div style={{
							...
							<p style={{ margin: 0, fontWeight: "bold", paddingRight: "50px" }}>{`Connect the room ${this.roomname}`}</p>
							<Input placeholder="Username" value={this.state.username} onChange={e => this.handleUsername(e)} />

							<Input.Password placeholder="Password room" value={this.state.password} onChange={e => this.handlePassword(e)} />
